...
exports.computeContextIsolation = computeContextIsolation;
exports.computeDecisionTraceability = computeDecisionTraceability;
exports.computeHandoffEfficiency = computeHandoffEfficiency;
exports.computeRuleCompliance = computeRuleCompliance;
exports.computeKnowledgePersistence = computeKnowledgePersistence;
 ...
 */
function computeContextIsolation(input) {
    var _a;
    const timeline = (0, validation_1.validateTimeline)(input);
 ...
 * Returns a value between 0 and 1.
 */
function computeDecisionTraceability(input) {
    const timeline = (0, validation_1.validateTimeline)(input);
    if (!timeline.events || timeline.events.length === 0) {
        throw new Error("Timeline must contain at least one event.");
    }
    const decisionEvents = timeline.events.filter((evt) => evt.type === "DECISION");
    if (decisionEvents.length === 0)
        return 1; // No decisions = fully traceable
    let traceable = 0;
 ...
 */
function computeHandoffEfficiency(input, delayThresholdMs = 60000) {
    const timeline = (0, validation_1.validateTimeline)(input);
    const handoffEvents = timeline.events.filter((evt) => evt.type === "HANDOFF");
    if (handoffEvents.length === 0)
        return 1; // No handoffs = fully efficient
    let efficient = 0;
 ...
 * Returns a value between 0 and 1.
 */
function computeRuleCompliance(input) {
    const timeline = (0, validation_1.validateTimeline)(input);
    if (!timeline.events || timeline.events.length === 0) {
        throw new Error("Timeline must contain at least one event.");
    }
 ...
 * Returns a value between 0 and 1.
 */
function computeKnowledgePersistence(input) {
    const timeline = (0, validation_1.validateTimeline)(input);
    if (!timeline.events || timeline.events.length === 0) {
        throw new Error("Timeline must contain at least one event.");
    }
    let persistent = 0;
    for (const evt of timeline.events) {
        const hasKnowledgeTag = Array.isArray(evt.tags) && evt.tags.includes("knowledge");
        const payload = evt.payload;
        const hasKnowledgeField = payload && payload.hasKnowledge === true;
